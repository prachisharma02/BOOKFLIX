...
//getting quantity from cart
 ...
 values ($user_id,$subtotal,$invoice_number,$count_product,'sysdate','$status')";

 $result_query=mysqli_query($con,$insert_orders);
 if($result_query)
 {
    // Function call
    function_alert("Orders are submitted successfully");
 ...
 if($result_delete)
 {
 $empty_orders="delete from`orders_pending` where ip_address='$get_ip_address'";
 $result_order=mysqli_query($con, $empty_orders);
 if($result_order)
 {
    echo "<h3>You have 0 Pending orders</h3>
    <a style='color:green' href='../homepage.php?My_orders'>Explore products</a>";
 }
 $empty_pending="delete from`user_orders` where ip_address='$get_ip_address'";
 $result_pending=mysqli_query($con, $empty_pending);
}
?>
